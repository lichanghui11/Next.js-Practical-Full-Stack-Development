...
- docker 是一个管理镜像的工具，镜像是死的，并没有运行，需要执行运行命令之后才会启动对应的容器，
- 镜像都会为不同的系统配备相应的包，不管在什么系统启动，都会安装最适合这个系统的包，解决了兼容问题
- 最便捷的方式是使用 docker-compose.yml 文件来整合所有需要使用的镜像
- 实际应用的时候我使用了 orbstack 的容器服务，没有自己运行命令
