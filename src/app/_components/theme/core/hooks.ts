...
export const useThemeActions = () => {
  ...
  const changeMode = useMemo(() => {
    return debounce(
      (v: `${ThemeMode}`) =>
        dispatch({ type: ThemeActions.CHANGE_MODE, payload: v }),
      500,
    );
  }, [dispatch]);
  ...
  const toggleMode = useMemo(() => {
    return debounce(() => dispatch({ type: ThemeActions.TOGGLE_MODE }), 500);
  }, [dispatch]);

  // 设置紧凑模式
  const changeCompact = useCallback(
    (v: boolean) => dispatch({ type: ThemeActions.CHANGE_COMPACT, payload: v }),
    [dispatch],
  );
