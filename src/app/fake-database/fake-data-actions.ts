...
import { readDbFile, resetDbFile } from './fake-data-generator';
...
// 根据 id 查询文章信息
export const queryPostById = async (id: string): Promise<IPost | null> => {
...
// 新增文章
export const addPost = async (post: Omit<IPost, 'id'>): Promise<IPost> => {
  const posts = await readDbFile();
  ...
  const newPost = {
    ...
    thumbnail: post.thumbnail || `/blog-demo-images/blog-${Math.floor(Math.random() * 11) + 1}.png`,
  } as IPost;
...
export const updatePost = async (post: Partial<IPost> & { id: string }): Promise<IPost | null> => {
  const posts = await readDbFile();

  const currPost = posts.find((item) => item.id === post.id) || null;
  if (isNil(currPost)) throw new Error('There is no such post!');
  ...
  await resetDbFile(newPosts);
  return newPosts.find((item) => item.id === post.id) || null;
};
