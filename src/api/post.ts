...
export const blogApi = {
  ...
  list: async (options: PostPaginateRequestQuery): Promise<Response> => {
    ...
    return fetchApi(blogClient, async (client) => {
  ...
  detail: async (item: string): Promise<Response> =>
    fetchApi(blogClient, async (client) => client[':item'].$get({ param: { item } })),

  // 通过 slug 查询某篇文章
  detailBySlug: async (slug: string): Promise<Response> =>
    fetchApi(blogClient, async (client) => client.bySlug[':slug'].$get({ param: { slug } })),

  // 通过 id 查询某篇文章
  detailById: async (id: string): Promise<Response> =>
    fetchApi(blogClient, async (client) => client.byId[':id'].$get({ param: { id } })),
  ...
  update: async (id: string, data: PostCreateOrUpdateData): Promise<Response> =>
    fetchApi(blogClient, async (client) =>
      client[':id'].$patch({ param: { id }, json: { ...data } }),
    ),

  // 删除文章
  delete: async (id: string): Promise<Response> =>
    fetchApi(blogClient, async (client) => client[':id'].$delete({ param: { id } })),
};
